...
TermBuffer.prototype.inject = function(str) {
	...
	for(i = 0; i < lines.length; i++) {
		...
		if(lines[i].indexOf('\r') != -1) {
			...
			lines[i] = this.getLine().str;
			c.x = 0;
			for(j = 0; j < line.length && lines[i].length < this.width; j++) {
...
TermBuffer.prototype._removeLine = function(line, count) {
	...
	var remove = this._buffer.attr.splice(line, count);
	for(i = 0; i < count; i++)
		this.emit('lineremove', line, remove[i]);
	return count;
};
