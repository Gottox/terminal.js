...
function mode(s) {
	return function(cmd, value) {
		this.buffer.setMode(s, value);
	};
}
module.exports = {
	// '0': // Error this command is ignored
	'1': mode('appKeypad'), // Application Key Pad - Guarded Area Transmit Mode, send all (VT132) (GATM)
	...
	// '?3': mode('132col'), // Column mode - 132 col (DECOLM)
	'4': mode('insert'), //  Insert/Replace Mode (IRM)
	// '?4': , // Scrolling Mode - Smooth (DECSCLM)
	//'5': // Status Report Transfer Mode, report after DCS (STRM)
	'?5': mode('reverse'), // Screen Mode - Reverse (DECSCNM)
	//'?6': mode('relative'), // Origin Mode, line 1 is relative to scroll region (DECOM)
	'7': mode('wrap'), // Wraparound - On - Vertical Editing Mode (VEM)
	//'?7': // AutoWrap Mode, start newline after column 80 (DECWAM)
	//'8': // reserved
	//'?8': // Auto Repeat Mode, key will autorepeat (DECARM)
	//'9': // reserved
	//'?9': mode('interlace'), // INterLace Mode, interlaced for taking photos
	//'10': // (HEM)
	...
	'20': mode('crlf'), // Automatic Linefeed Mode (LNM)
	...
	'?25': mode('cursor'), // Visible Cursor (DECTCEM)
	...
	'?47': function(cmd, value) {
		this.buffer.buffer = value ? this.buffer.altBuffer : this.buffer.defaultBuffer;
	},
	...
	'?1047': function(cmd, value) {
		// TODO move this to TermBuffer
		this.buffer.altBuffer = [];
		this.buffer.buffer = value ? this.buffer.altBuffer : this.buffer.defaultBuffer;
	},
	'?1048': function(cmd, v) {
		if(v)
	...
	'?1049': function(cmd, v) {
		this.callHandler('mode', '1048');
		this.callHandler('mode', '1047');
		if(v)
