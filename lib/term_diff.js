...
TermDiff.prototype._mkSize = function(oldBuffer, newBuffer){
	if(oldBuffer.columns !== newBuffer.columns || oldBuffer.rows !== newBuffer.rows) {
		this._rows = newBuffer.rows;
		this._columns = newBuffer.columns;
	}
};
...
TermDiff.prototype._mkDiff = function(oldBuffer, newBuffer) {
	var m = Math.max(1, oldBuffer.getBufferRowCount()),
	n = Math.max(1, newBuffer.getBufferRowCount());
