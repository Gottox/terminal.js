...
DomInput.prototype._registerEvents = function() {
	...
	this._addListener(target, 'click', this._focus);
	this._addListener(catcher, 'focus', this._focus);
	this._addListener(catcher, 'blur', this._blur);
	this._addListener(catcher, 'keypress', this._keypress);
	this._addListener(catcher, 'keydown', this._keydown);
};
...
DomInput.prototype.getKey = function(ev) {
	if(!ev)
		return;
	else if(ev.charCode)
		return ev.charCode;
...
DomInput.prototype._keydown = function(ev) {
	var key, keyCode = this.getKey(ev);
