...
Terminal.prototype.createChar = function(chr) {
	this.attrCommited = true;
...
Terminal.prototype.createLine = function() {
	return {line: [], attr: {}};
}

Terminal.prototype.getLine = function() {
	var c = this.cursor;
	return this.buffer[c.y] || (this.buffer[c.y] = this.createLine());
}

Terminal.prototype.write = function(data, encoding) {
...
Terminal.prototype.escapeWrite = function(data) {
	var cmd = data[1];
...
Terminal.prototype.inject = function(str) {
...
Terminal.prototype.mvCur = function(x, y) {
	return this.setCur({
...
Terminal.prototype.setCur = function(cur) {
	var inbound = 0;
	if(cur.x < 0)
		cur.x = 0;
...
Terminal.prototype.mvTab = function(n) {
...
Terminal.prototype.tabSet = function(pos) {
...
Terminal.prototype.tabUnset = function(pos) {
...
Terminal.prototype.setTabs = function(tabs) {
...
Terminal.prototype.tabClear = function(n) {
	switch(n || 'current') {
		case 'current':
		case 0:
...
Terminal.prototype.setSavedCursor = function(cursor) {
...
Terminal.prototype.deleteCharacters = function(n) {
	var line = this.getLine().line;
	line.splice(this.cursor.x, n || 1);
}

Terminal.prototype.eraseCharacters = function(n) {
	this.deleteCharacters(n);
	this.insertBlanks(n);
}

Terminal.prototype.setScrollRegion = function(n, m) {
...
Terminal.prototype.resetAttr = function() {
...
Terminal.prototype.chAttr = function(name, value) {
...
Terminal.prototype.insertBlanks = function(n) {
	var line = this.getLine();
	line.changed = true;
...
Terminal.prototype.insertLines = function(n, pos) {
	if(pos === undefined)
		pos = this.cursor.y;
...
Terminal.prototype.toString = function(locateCursor) {
	var ret = []
	if(locateCursor) {
...
Terminal.prototype.setLed = function(n,value) {
	if(n == 0)
		this.leds = [!!0,!!0,!!0,!!0];
	else
	if (value === undefined) {
...
Terminal.prototype.setLeds = function(leds) {
	for(var i in leds) {
		this.setLed(i,leds[i]);
	}
}

Terminal.prototype.setModes = function(mode) {
	for(var i in mode) {
		this.mode[i] = mode[i];
	}
},

Terminal.prototype.setDefaultAttr = function(attrs) {
	for(var i in attrs) {
		this.defaultAttrs[i] = attrs[i];
	}
}

Terminal.prototype.setLineAttr = function(attrs) {
	for(var i in attrs) {
		this.lineAttr[i] = attrs[i];
	}
}

Terminal.prototype.scroll = function(dir, lines) {
	if(lines === undefined)
		lines = 1;
	// TODO hacky!
...
Terminal.prototype.eventToKey = function(event) {
...
Terminal.prototype.diff = function(otherTerm) {
...
Terminal.prototype.apply = function(diff) {
