...
function DomOutput(buffer, writer, target, opts) {
	...
	target.innerHTML = '<div style="visibility:hidden;line-height:0"></div>';
...
DomOutput.prototype._updateRowCount = function() {
	var diff = this.buffer.rows - this.buffer.getBufferRowCount();

	var html = myUtil.repeat('&nbsp;', this.buffer.columns) +
		myUtil.repeat('<br />&nbsp;', diff-1);
