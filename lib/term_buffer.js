...
TermBuffer.prototype.getLine = function() {
	var c = this.cursor;
	if(!this.buffer[c.y]) {
		this.buffer[c.y] = this.createLine();
		this.emit('lineinsert', c.y, this.buffer[c.y]);
	}
	return this.buffer[c.y];
};
...
TermBuffer.prototype.inject = function(str, len) {
	...
	this.silent = false;
	this.emit("linechange", c.y, line);
	this.emit("cursormove", this.cursor);
};
...
TermBuffer.prototype.insertLines = function(n, pos) {
	...
	args.unshift(pos, 0);
	Array.prototype.splice.apply(this.buffer, args);
	var rm = this.buffer.splice(this.scrollRegion[1], this.buffer.length - this.scrollRegion[1] - tail);
	for(i = 0; i < rm.length; i++)
		this.emit('lineremove', this.scrollRegion[1], rm[i]);
	for(i = 0; i < n; i++)
		this.emit('lineinserted', this.scrollRegion[1], undefined);
};
...
TermBuffer.prototype.scroll = function(dir, lines) {
	if(lines === undefined)
		lines = 1;
	// TODO hacky!
	var rmIndex = this.scrollRegion[dir === 'up' ? 1 : 0];
	var insIndex = this.scrollRegion[dir === 'up' ? 0 : 1];
	for(var i = 0; i < lines; i++) {
		var newLine = this.createLine();
		this.buffer.splice(insIndex, 0, newLine);
		this.emit('lineinsert', insIndex, newLine);
		if(this.buffer.length > rmIndex) {
			var rm = this.buffer.splice(rmIndex, 1);
			this.emit('lineremove', rmIndex, rm[0]);
		}
	}
};
