...
TermBuffer.prototype._lineInject = function(content) {
	...
	if(this._modes.insert) {
		...
		args.unshift(c.x, 0);
		l.attr.splice.apply(l.attr, args);
		l.attr.splice(this.width);
		l.str = l.str.substr(0, c.x) + this._mkBlanks(c.x - l.str.length) +
			content + l.str.substr(c.x);
		l.str = l.str.substr(0, this.width);
	}
	else {
		l.str = l.str.substr(0, c.x) + this._mkBlanks(c.x - l.str.length) +
			content + l.str.substr(c.x+content.length);
	}
	
	this._applyAttributes(l.attr, c.x, content.length);
	this._setLine(c.y, l);
...
TermBuffer.prototype.eraseInLine = function(n) {
	var c = this._cursor;
	var l = this.getLine(c.y);
	switch(n || 0) {
		case 'toRight':
		case 0:
			l.str = l.str.substr(0, c.x);
			// TODO change attr accordingly
			break;
		case 'toLeft':
		case 1:
			l.str = this._mkBlanks(c.x) + l.str.substr(c.x);
			// TODO change attr accordingly
			break;
		case 'all':
		case 2:
