...
TermBuffer.prototype.removeChars = function(count) {
	var c = this.cursor, line = this.getLine(c.y);
	line.str = line.str.substr(0, c.y) + line.str.substr(c.y+count);
...
TermBuffer.prototype.setCursor = function(x, y) {
	...
	if(c.x != x || c.y != y || arguments.length === 0) {
		...
		c.x = x;
		c.y = y;
		this.emit('cursormove', x, y, oldX, oldY);
	}
