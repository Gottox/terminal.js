...
function Terminal(width, height, attr) {
  ...
  // TODO - this should read the (Default)LineAttr
	...
	this.escapeWrite = function(data) {
		var cmd = data[1];
...
this.inject = function(str) {
  ...
  // Removes previous set tabs if they are not specified
	...
	this.setSavedCursor= function(cursor) {
	...
	this.deleteCharacters= function(n) {
		var line = this.getLine().line;
		line.splice(this.cursor.x, n || 1);
	}

	this.eraseCharacters= function(n) {
		this.deleteCharacters(n);
		this.insertBlanks(n);
	}

	this.setScrollRegion= function(n, m) {
		this.scrollRegion[0] = n;
		this.scrollRegion[1] = m;
	}

	this.eraseInDisplay= function(n) {
		switch(n || 0) {
		case 0:
	...
	this.addLine= function(line, row) {
    // TODO - figure out to reuse InsertLines
    this.buffer.splice(row,0,line);
  }
	...
	this.deleteLines= function(n, startRow, bla) {
    var row = startRow;
    if (row === undefined) {
      row = this.cursor.y
    }
	...
	this.chAttr= function(name, value) {
	...
	this.insertLines= function(n, pos) {
		if(pos === undefined)
			pos = this.cursor.y;
	...
	this.toString= function(locateCursor) {
		var ret = []
		if(locateCursor) {
	...
	this.setModes= function(mode) {
    for(var i in mode) {
      this.mode[i] = mode[i];
    }
  },

	this.setDefaultAttr= function(attrs) {
    for(var i in attrs) {
      this.defaultAttrs[i] = attrs[i];
    }
  }

	this.setLineAttr= function(attrs) {
    for(var i in attrs) {
      this.lineAttr[i] = attrs[i];
    }
  }

	this.scroll= function(dir, lines) {
		if(lines === undefined)
			lines = 1;
		// TODO hacky!
	...
	this.eventToKey= function(event) {
...
util2.inherits(Terminal, events.EventEmitter);
